...
function buildWritebackPrompt(context: ExistingAssetContext): string {
  ...
  return [
    ...
    `现有伏笔：${JSON.stringify(context.foreshadows.slice(0, 40).map((item) => ({ id: item.id, title: item.title, status: item.status, targetPayoffChapter: item.targetPayoffChapter })))}`,
