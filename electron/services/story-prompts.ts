...
const CHAPTER_DELIVERY_GATE_LINES = [
  ...
  '- 黄金三章状态门禁：第 2 章不能只扩写第 1 章的谜团，第 3 章不能只增加危险；章尾钩子要在局部真相、误判、损失、关系破裂和新危险之间轮换。',
  ...
  const fallback = appendPromptText(
    appendPromptSection(withStructuralAlerts, '补充审校要求', [
      ...
      ...(isKeyTier(promptTier)
        ...
        : []),
      '- 证据要求：critical_fixes、continuity_risks、language_risks 等所有风险列表条目，都必须在条目末尾追加「【证据】逐字摘抄的正文短句(20-80字)」；证据必须原样摘抄正文，不得改写、缩写或转述。找不到可回指证据时不要输出该风险，信息不足则在 summary 中说明无法判断。',
      '- 额外输出顶层字段 verdicts（语义评审格式）：对 contract_delivery（合同兑现）、structural_beat（结构节拍）、cost_and_choice（代价与选择）、supporting_agency（配角主体性）、dialogue_voice（对白声纹）五个维度各给一条判定；status 只能是 pass/warning/blocker/uncertain，每条判定的 evidence 必须逐字摘抄正文短句，找不到证据时 status 用 uncertain，不得凭印象给 pass 或 blocker。',
    ]),
