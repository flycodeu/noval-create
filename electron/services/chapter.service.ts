...
const MAX_STRUCTURAL_REPAIR_EXPANSION_RATIO = 1.6
...
const GENERIC_NARRATIVE_OBJECT_MARKERS = [
  '钥匙', '复印件', '文件袋', '照片', '语音', '签名', '文件', '记录',
]
const MAX_INJECTED_NARRATIVE_OBJECTS = 16

function collectTrackedObjectNames(novelId: number): string[] {
  const db = getDb()
  const names = [
    ...db.select({ name: storyItems.itemName }).from(storyItems).where(eq(storyItems.novelId, novelId)).all().map((row) => row.name),
    ...db.select({ name: glossary.term }).from(glossary).where(eq(glossary.novelId, novelId)).all().map((row) => row.name),
...
function buildNarrativeObjectMarkers(novelId: number): string[] {
...
function collectProtagonistAliases(novelId: number): string[] {
  const db = getDb()
  const rows = db
...
function collectSupportingCastNames(novelId: number): string[] {
  const db = getDb()
  const rows = db
...
interface NarrativeGuardContext {
  protagonistAliases: string[]
  objectMarkers: string[]
}

const DEFAULT_NARRATIVE_GUARD_CONTEXT: NarrativeGuardContext = {
  protagonistAliases: [],
  objectMarkers: GENERIC_NARRATIVE_OBJECT_MARKERS,
}

function buildNarrativeGuardContext(novelId: number): NarrativeGuardContext {
  return {
...
const UNSUPPORTED_NARRATIVE_FACT_PATTERNS: Array<{ key: string; label: string; pattern: RegExp }> = [
  {
...
function narrativeTailSentences(text: string): string[] {
  return String(text || '')
    .split(/[。！？!?；;\n]+/u)
...
type NarrativeObjectState = 'held' | 'reversed'

function lastNarrativeObjectState(sentences: string[], objectName: string): NarrativeObjectState | null {
...
function lastNarrativeExitState(sentences: string[], guardContext: NarrativeGuardContext): 'permissive' | 'blocking' | null {
...
function collectNarrativeStateWarnings(
...
function collectUnsupportedNarrativeFactWarnings(originalContent: string, optimizedContent: string): string[] {
...
function buildChapterOptimizationFactGuard(
  novelId: number,
  originalContent: string,
  optimizedContent: string,
