...
function buildHandoffExtractionPrompt(input: CreateChapterEndHandoffDraftInput): string {
  return [
    ...
    '规则：',
    '- changes 只写本章已经发生的状态变化、关系变化、资源变化或剧情推进，最多 6 条；只是安静相处而没有新变化时可以为空。',
    '- costs 只写本章已经付出的代价，不要把未来风险冒充已发生事实。',
    '- open_questions 只写本章结束仍未解决、且正文确实提出的问题。',
    '- next_pressure 写下一阶段有依据的余波或待回应事项；照护安排完成后的相处、满足和配角工作也可以承接，允许留空，不为转阶段编造危机。',
    '- 区分提出、进展、回应和有原因的延后；只记录正文实际回应的具体约定，取得资格不代表比赛或夺冠已经完成。人物误信仍是认识，不把候选计划当事实。',
