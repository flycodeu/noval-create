...
export function installWebElectronBridge(): void {
  ...
  const bridge = {
    ...
    narrativeBoard: createService('narrativeBoard', {
      getSnapshot: async (scope?: unknown) => withLocalBackend('narrativeBoard', 'getSnapshot', [scope], async () => ({
        novelId: Number((scope as { novelId?: unknown } | undefined)?.novelId || 0),
        scope: scope || {},
        tree: [],
        mapRelations: [],
        mapLayout: [],
        viewport: {
          ...
          centerY: 0,
          zoom: 1,
          activeLayers: ['regions', 'routes', 'events', 'people', 'factions'],
          layoutVersion: 1,
          contextVersion: 1,
        },
        ...
        totals: { map: 0, characters: 0, relations: 0, events: 0, tasks: 0, threads: 0, unresolvedAnchors: 0 },
      })),
      getLayout: async (novelId?: unknown, layoutKey?: unknown, mapNodeIds?: unknown) => withLocalBackend('narrativeBoard', 'getLayout', [novelId, layoutKey, mapNodeIds], async () => []),
      upsertLayoutNode: (...args) => withLocalBackend('narrativeBoard', 'upsertLayoutNode', args, async () => readOnlyMutation('narrativeBoard.upsertLayoutNode')),
      saveViewport: (...args) => withLocalBackend('narrativeBoard', 'saveViewport', args, async () => readOnlyMutation('narrativeBoard.saveViewport')),
      upsertLocationBinding: (...args) => withLocalBackend('narrativeBoard', 'upsertLocationBinding', args, async () => readOnlyMutation('narrativeBoard.upsertLocationBinding')),
      listLocationBindings: async (scope?: unknown) => withLocalBackend('narrativeBoard', 'listLocationBindings', [scope], async () => []),
    }),
