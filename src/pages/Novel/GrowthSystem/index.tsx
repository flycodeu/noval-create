...
export default function GrowthSystemPage({ novelId }: Props) {
  ...
  const [bindChapterTrackIds, setBindChapterTrackIds] = useState<number[]>([])
  const [bindChapterPoolIds, setBindChapterPoolIds] = useState<number[]>([])
  const [bindVolumeTrackIds, setBindVolumeTrackIds] = useState<number[]>([])
  const [bindVolumePoolIds, setBindVolumePoolIds] = useState<number[]>([])
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setBindChapterTrackIds((current) => current.filter((id) => nextTrackIds.has(id)))
      setBindVolumeTrackIds((current) => current.filter((id) => nextTrackIds.has(id)))
      setBindChapterPoolIds((current) => current.filter((id) => nextPoolIds.has(id)))
      setBindVolumePoolIds((current) => current.filter((id) => nextPoolIds.has(id)))
    ...
    >
          ...
          {activeSection === 'tracks' && selectedTrack ? (
            <div data-growth-current-detail>
              ...
              <div className="novel-growth-system__detail-facts">
                ...
                <div><span>关联章节</span><strong>{selectedTrack.linkedChapterId ? (chapters.find((chapter) => chapter.id === selectedTrack.linkedChapterId)?.chapterNum ? `第${chapters.find((chapter) => chapter.id === selectedTrack.linkedChapterId)?.chapterNum}章` : `章节#${selectedTrack.linkedChapterId}`) : '未绑定'}</strong></div>
              </div>
      ...
      <details className="novel-growth-system__binding-disclosure" data-growth-binding>
        ...
        <div className="novel-growth-system__binding-grid">
          <div className="novel-growth-system__binding-group"><strong>绑定章节合同</strong><Select value={bindChapterId || undefined} onChange={(value) => setBindChapterId(value || null)} options={chapters.map((chapter) => ({ value: chapter.id, label: `第${chapter.chapterNum}章 · ${chapter.title || '未命名章节'}` }))} placeholder="选择章节" /><Select mode="multiple" value={bindChapterTrackIds} onChange={(value) => setBindChapterTrackIds(value as number[])} options={tracks.map((track) => ({ value: track.id, label: track.title }))} placeholder="成长轨道" /><Select mode="multiple" value={bindChapterPoolIds} onChange={(value) => setBindChapterPoolIds(value as number[])} options={pools.map((pool) => ({ value: pool.id, label: pool.name }))} placeholder="资源池" /><Select mode="multiple" value={bindEventIds} onChange={(value) => setBindEventIds(value as number[])} options={events.map((event) => ({ value: event.id, label: `第${event.chapterNumSnapshot || '?'}章 · ${event.title}` }))} placeholder="回写事件" /><Button type="primary" disabled={!bindChapterId} onClick={() => bindChapterId && void window.electron.growthSystem.bindChapterContract(novelId, { chapterId: bindChapterId, trackIds: bindChapterTrackIds, poolIds: bindChapterPoolIds, eventIds: bindEventIds }).then(() => message.success(getUserFacingMessage('growthSystem.chapterBound'))).catch((error) => message.error(getErrorMessage(error, 'common.saveFailed')))}>绑定章节合同</Button></div>
          <div className="novel-growth-system__binding-group"><strong>绑定卷级节奏</strong><Select value={bindVolumeId || undefined} onChange={(value) => setBindVolumeId(value || null)} options={volumes.map((volume) => ({ value: volume.id, label: volume.title?.trim() || `第${volume.volumeNumber}卷` }))} placeholder="选择卷" /><Select mode="multiple" value={bindVolumeTrackIds} onChange={(value) => setBindVolumeTrackIds(value as number[])} options={tracks.map((track) => ({ value: track.id, label: track.title }))} placeholder="卷级轨道" /><Select mode="multiple" value={bindVolumePoolIds} onChange={(value) => setBindVolumePoolIds(value as number[])} options={pools.map((pool) => ({ value: pool.id, label: pool.name }))} placeholder="卷级资源池" /><Input.TextArea rows={3} value={bindCadence} onChange={(event) => setBindCadence(event.target.value)} placeholder="卷级奖励节奏约束说明" /><Button type="primary" disabled={!bindVolumeId} onClick={() => bindVolumeId && void window.electron.growthSystem.bindVolumeDesign(novelId, { volumeId: bindVolumeId, trackIds: bindVolumeTrackIds, poolIds: bindVolumePoolIds, rewardCadence: text(bindCadence) || undefined }).then(() => message.success(getUserFacingMessage('growthSystem.volumeBound'))).catch((error) => message.error(getErrorMessage(error, 'common.saveFailed')))}>绑定卷级节奏</Button></div>
        </div>
      </details>
