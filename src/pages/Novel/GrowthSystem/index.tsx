...
export default function GrowthSystemPage({ novelId }: Props) {
  ...
  const saveTrack = async () => {
    ...
    if (!text(values.title)) return message.warning('请填写轨道标题。')
    setSaving(true)
    try {
  ...
  const savePool = async () => {
    ...
    if (!text(values.name)) return message.warning('请填写资源池名称。')
    setSaving(true)
    try {
  ...
  const saveEvent = async () => {
    ...
    if (!text(values.title)) return message.warning('请填写回写标题。')
    setSaving(true)
    try {
