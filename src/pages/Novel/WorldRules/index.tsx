      ...
      <WorkspacePanel title="分区编辑器" extra={<div className="novel-pill">{`当前分区：${activeSectionMeta.label}`}</div>}>
        <AiPatchEditor
          ...
          onApplied={async (applied) => {
            const updated = applied as Awaited<ReturnType<typeof window.electron.novel.get>>
            if (updated) {
              setCurrentNovel(updated)
              form.setFieldsValue(parseWorldRulesDraftJson(updated.worldRulesJson, currentNovel?.genreName))
            }
            await window.electron.worldRules.clearAutoGenerateDraft(novelId)
            setAutoTask(null)
            setAutoStatus(EMPTY_AUTO_STATUS)
