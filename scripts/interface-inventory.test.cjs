...
const webBridgeSource = fs.readFileSync(path.join(workspaceRoot, 'src/runtime/web-electron-bridge.ts'), 'utf8')
const toolContractSource = fs.readFileSync(path.join(workspaceRoot, 'src/shared/tool-contracts/index.ts'), 'utf8')
...
assert.deepEqual(inventory.crossSurface.desktopOnly, [])
assert.deepEqual(inventory.crossSurface.localWebOnly, [])
assert.equal(inventory.crossSurface.mirroredBusinessApis, inventory.crossSurface.expectedWebBusinessApis)
assert.equal(LOCAL_WEB_BACKEND_VERSION, 6)
...
assert.match(localBackendSource, /approve:\s*\(approvalPayload\)/u)
assert.match(localBackendSource, /consumeApprovalGrant/u)
assert.match(webBridgeSource, /withLocalBackend\('agentTools',\s*'approve'/u)
assert.match(toolContractSource, /AGENT_TOOL_SCOPES\.canonWrite/u)
assert.match(toolContractSource, /AGENT_TOOL_SCOPES\.recommendationRecord/u)
